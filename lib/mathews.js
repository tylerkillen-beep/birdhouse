...
/** True for the drink's default size (16 oz): already included, so it's shown
 *  but not pickable. Only 24 oz is an upgrade, and it counts as a modifier. */
function isMathewsDefaultSize(list, option) {
  const norm = s => String(s || '').trim().toLowerCase().replace(/\s+/g, ' ');
  return norm(list?.name) === 'size' && norm(option?.name) === '16 oz';
}
